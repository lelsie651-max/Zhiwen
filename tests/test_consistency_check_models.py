...
def test_consistency_check_tables_compile_with_postgresql_offline_ddl() -> None:
    ...
    assert "fk_ccapp_srcapp_fvcca" in app_sql
...
def test_consistency_check_composite_foreign_keys_and_restricts() -> None:
    ...
    assert app_foreign_keys[("consistency_application_id",)] == (
        "fk_ccapp_srcapp_fvcca",
        ("id",),
        "RESTRICT",
    )
    ...
    assert batch_foreign_keys[("inference_run_id",)] == (
        "fk_ccbatch_inference_run_id_ir",
        ("id",),
        "RESTRICT",
    )
    ...
    assert citation_foreign_keys[("assessment_id",)] == (
        "fk_cccite_assessment_id_ccasmt",
        ("id",),
        "RESTRICT",
    )
    assert citation_foreign_keys[("evidence_link_id",)] == (
        "fk_cccite_evidence_link_id_fel",
        ("id",),
        "RESTRICT",
    )
...
def test_consistency_check_constraints_cover_batch_shape_and_contract_checks() -> None:
    batch_checks = {
    ...
    impact_sql = next(
        sql for name, sql in assessment_checks.items() if name is not None and name.endswith("impact_json_is_array")
    )
    actions_sql = next(
        sql
        for name, sql in assessment_checks.items()
        if name is not None and name.endswith("recommended_actions_json_is_array")
    )
    ...
    assert "jsonb_typeof(impact_json) = 'array'" in impact_sql
    assert "jsonb_typeof(recommended_actions_json) = 'array'" in actions_sql
